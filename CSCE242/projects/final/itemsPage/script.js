...
const deleteItem = async (item) => {
    let response = await fetch (`http://localhost:3000/api/items/${item._id}`, {
        method: "DELETE",
        headers: {
...
const getJSON = async () => {
    try{
        return (await fetch("http://localhost:3000/api/items")).json();
    }
    catch(error){
        console.log(error);
    }
...
const addEditItem = async (e) => {
    ...
    response = await fetch("http://localhost:3000/api/items", {
        ...
        response = await fetch(`http://localhost:3000/api/items/${form._id.value}`, {
            method: "PUT",
            body: formData
        });
    }
